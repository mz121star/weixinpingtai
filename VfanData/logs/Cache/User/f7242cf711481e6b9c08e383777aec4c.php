...
</script><script>function selectall(name) {
...
</script><link rel="stylesheet" type="text/css" href="<?php echo RES;?>/css/cymain.css" /><div class="content"><div class="cLineB"><h4 class="left">当前位置：<a href="<?php echo U('Member_card/index',array('token'=>$token));?>">会员卡</a> >><?php echo ($thisCard["cardname"]); ?>：创建会员卡号　<span style="color:red">此会员卡号,会员领取一张，这里将会少一张,当全部被领取完了,请再次开卡</span></h4><br /><div class="clr"></div></div><div class="cLine"><div class="pageNavigator left"><a href="<?php echo U('Member_card/create_add',array('token'=>$token,'id'=>$thisCard['id']));?>" title="创建会员卡号" class="add">创建会员卡号
				</a>&nbsp;&nbsp;<a class="delete" href="###" onclick="if(confirm('确定删除吗')){$('#info').submit()}">删除</a>&nbsp;&nbsp; <strong>已经领取 <span style="color:red"><?php echo ($usecount); ?></span> 张，剩余<span style="color:red"><?php echo ($ucount); ?></span>张未领取，一共 <span style="color:red"><?php echo ($count); ?></span>张</strong></div><a href="javascript:history.go(-1);" class="right btnGrayS vm" style="margin-top:10px">返回</a><div class="clr"></div></div><div class="msgWrap"><form method="post" action="" id="info"><input name="delall" type="hidden" value="del"><input name="wxid" type="hidden" value="gh_423dwjkewad"><table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%"><thead><tr><th class="select"><input type="checkbox" value="" id="check_box" onclick="selectall('id[]');"></th><th class="category">会员卡号</th><th class="edit norightborder">状态</th><th class="category">会员资料</th></tr></thead><tbody><?php if(is_array($data_vip)): $i = 0; $__LIST__ = $data_vip;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data_vip): $mod = ($i % 2 );++$i;?><tr><td><input type="checkbox" value="<?php echo ($data_vip["id"]); ?>" class="cbitem" name="id_<?php echo ($i); ?>"></td><td><div class="answer_text"><?php echo ($data_vip["number"]); ?></div></td><td class="norightborder"><!--a href="">使用统计</a--><?php if($data_vip['wecha_id'] == false): ?>空闲卡<?php else: ?><strong>使用中</strong><?php endif; ?></td><td><?php if($data_vip['wecha_id'] != false): ?><a href="index.php?g=User&m=Member_card&a=member&itemid=<?php echo ($data_vip["id"]); ?>&id=<?php echo ($thisCard["id"]); ?>">查看详情</a><?php endif; ?></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table></form><script>			   function checkvotethis() {
