...
</script><div class="content"><div class="cLineB"><h4 class="left">当前位置：<a href="<?php echo U('Member_card/index',array('token'=>$token));?>">会员卡</a> >><?php echo ($thisCard["cardname"]); ?>：会员优惠卷管理</h4><div class="clr"></div></div><div class="cLine"><div class="pageNavigator left"><a href="<?php echo U('Member_card/coupon_edit',array('token'=>$token,'id'=>$thisCard['id']));?>" title="发布优惠券" class="btnGrayS vm bigbtn"><img src="<?php echo RES;?>/images/cart_info/power2.png" width="24px" class="vm">发布优惠券
			  </a>　

			</div><a href="javascript:history.go(-1);" class="right btnGrayS vm" style="margin-top:10px">返回</a><div class="clr"></div></div><div class="msgWrap"><form method="post" action="index.php?ac=importtxt&amp;id=9878&amp;wxid=gh_423dwjkewad" id="info"><input name="delall" type="hidden" value="del"><input name="wxid" type="hidden" value="gh_423dwjkewad"><table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%"><thead><tr><th class="select">选择</th><th class="answer">标题</th><th class="time" style="width:80px;">使用次数</th><th class="category" style="display:none">会员级别</th><th class="time">创建时间</th><th class="time">过期时间</th><th class="edit norightborder">操作</th></tr></thead><tbody><?php if(is_array($data_vip)): $i = 0; $__LIST__ = $data_vip;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data_vip): $mod = ($i % 2 );++$i;?><tr><td><input type="checkbox" name="del_id[]" value="" class="checkitem"></td><td><div class="answer_text"><?php echo ($data_vip["title"]); ?></div></td><td><?php echo ($data_vip["usetime"]); ?></td><td style="display:none"><?php switch($data_vip["group"]): case "1": ?>普通会员<?php break; case "2": ?>银卡会员<?php break; case "3": ?>金卡会员<?php break; case "4": ?>钻石卡会员<?php break; endswitch;?></td><td><?php if($data_vip['type'] == 1): ?>无时间限制<?php else: echo (date('Y-m-d',$data_vip["statdate"])); endif; ?></td><td><?php if($data_vip['type'] == 1): ?>无时间限制<?php else: echo (date('Y-m-d',$data_vip["enddate"])); endif; ?></td><td class="norightborder"><a href="<?php echo U('Member_card/useRecords',array('id'=>$thisCard['id'],'type'=>'coupon','itemid'=>$data_vip['id']));?>">使用统计</a><a href="<?php echo U('Member_card/coupon_edit',array('itemid'=>$data_vip['id'],'id'=>$thisCard['id']));?>">编辑</a><a href="<?php echo U('Member_card/coupon_del',array('itemid'=>$data_vip['id'],'id'=>$thisCard['id']));?>">删除</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody></table></form><script>			   function checkvotethis() {
