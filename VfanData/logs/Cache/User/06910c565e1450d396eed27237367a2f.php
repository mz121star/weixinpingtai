...
</script><div class="content"><div class="cLineB"><h4><?php if($vo['type'] != 'img'): ?>文本<?php else: ?>图片<?php endif; ?>投票设置 </h4><a href="<?php echo U('Vote/index');?>" class="right btnGrayS vm" style="margin-top:-27px">返回</a></div><div class="msgWrap bgfc"><form class="form" method="post" action="" target="_top" enctype="multipart/form-data"><table class="userinfoArea" style=" margin:0;" border="0" cellspacing="0" cellpadding="0" width="100%"><tbody><tr><th width="120">关键词：</th><td><input type="text" name="keyword" value="<?php if($vo['keyword'] == ''): ?>投票<?php else: echo ($vo["keyword"]); endif; ?>" class="px" style="width:550px;"><br><span class="red">只能写一个关键词,功能面板必须开启投票</span></td></tr><tr><th width="120">投票标题：</th><td><input type="text" name="title" value="<?php echo ($vo["title"]); ?>" class="px" style="width:550px;"></td></tr><tr><th>投票图片：</th><td><img class="thumb_img" id="picurl_src" src="<?php echo (($reslist['picurl'])?($reslist['picurl']):'http://image.pigcms.com/yicms/2013/12/26/1388041408_1ed0f0591f583869.jpg'); ?>" style="max-height:100px;"><input type="text" name="picurl" value="<?php echo (($vo["picurl"])?($vo["picurl"]):'http://image.pigcms.com/yicms/2013/12/26/1388041408_1ed0f0591f583869.jpg'); ?>" class="px" onclick="document.getElementById('picurl_src').src=this.value;" id="picurl" style="width:300px;"><script src="/tpl/static/upyun.js"></script><a href="###" onclick="upyunPicUpload('picurl',700,400,'<?php echo ($vo["token"]); ?>')" class="a_upload">上传</a><a href="###" onclick="viewImg('picurl')">预览</a>&nbsp;填写图片外链地址，大小为720x400</td></tr><tr><th>图片显示：</th><td><label><input type="radio" name="showpic" value="1" checked="checked" id="showpic2">显示在投票页面</label><label><input name="showpic" type="radio" id="showpic1" value="0">不显示在投票页面</label></td></tr><tr><th valign="top">投票说明：</th><td valign="top"><textarea class="px" id="info" name="info" style="width: 560px; height: 120px; display: ;"><?php echo html_entity_decode(htmlspecialchars_decode($vo['info'])); ?></textarea></td></tr><tr><th valign="top">投票选项：</th><td valign="top"><div class="bdrcontent"><div id="div_ptype"><table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%"><tbody><tr><td width="50">序号</td><td width="260">选项标题</td><td width="50">排序</td><td width="50">票数</td><?php if($vo['type'] == 'img' OR $type == 'img'): ?><td width="260">图片外链地址</td><td width="260">图片跳转地址以http://开头</td><?php endif; ?><td class="norightborder"></td></tr></tbody><?php if($items != ''): ?><tbody><?php if(is_array($items)): $i = 0; $__LIST__ = $items;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ivo): $mod = ($i % 2 );++$i;?><tr><td width="50"><input type="text" name="add[id][]" readonly="readonly" disabled="disabled"  value="<?php echo ($ivo["id"]); ?>" style="width:50px;"></td><td width="260"><input type="text" name="add[item][]" value="<?php echo ($ivo["item"]); ?>" class="px" style="width:240px;"></td><td width="50"><input type="text" name="add[rank][]" value="<?php echo (($ivo["rank"])?($ivo["rank"]):1); ?>" style="width:50px;" class="px"></td><td width="50"><input type="text" name="add[vcount][]" value="<?php echo ($ivo["vcount"]); ?>" style="width:50px;" class="px"></td><?php if($vo['type'] == 'img' OR $type == 'img'): ?><td width="260"><input type="text" name="add[startpicurl][]" value="<?php echo ($ivo["startpicurl"]); ?>" style="width:50px;" class="px"></td><td width="260"><input type="text" name="add[tourl][]" value="<?php echo (($ivo["tourl"])?($ivo["tourl"]):'#'); ?>" style="width:50px;" class="px"></td><?php endif; ?><td width="50" class="norightborder"><a href="javascript:;" onclick="delrow(this, 'div_add_del');">删除</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?></tbody><?php endif; ?><tbody id="div_add_del" name="div_add_del"><tr style="display:none"><td width="50"><input type="text" name="add[id][]" readonly="readonly" disabled="disabled"  value="" style="width:50px;" ></td><td width="200"><input type="text" name="add[item][]" value="" class="px" style="width:150px;"></td><td><input type="text" name="add[rank][]" value="<?php echo (($ivo["rank"])?($ivo["rank"]):1); ?>" style="width:50px;" class="px"></td><td><input type="text" name="add[vcount][]" value="0" style="width:50px;" class="px"></td><?php if(($vo['type'] == 'img') OR ($type == 'img')): ?><td width="80"><input type="text" name="add[startpicurl][]" value="" class="px" style="width:190px;"></td><td width="100"><input type="text" name="add[tourl][]" value="#" class="px" style="width:100px;"></td><?php endif; ?><td width="50" class="norightborder"><a href="javascript:;" onclick="delrow(this, 'div_add_del');">删除</a></td></tr></tbody><tbody><tr><td colspan="4" class="norightborder"><?php if($items == ''): ?><button type="button" id="vtype" onclick="addrow('div_add_del');" class="btnGrayS vm" value="true"><strong>添加选项</strong></button>至少2项,最多15项,已经添加[<span id='aditem'></span>]项</td><?php else: ?><!--input type="button" readonly disabled value="添加选项"/--><?php endif; ?></tr></tbody></table><script>var count = 1;
...
}</script></div></div></td></tr><tr><th>单选/多选：</th><td><p style="width: 120px; float: left; display: block; line-height: 32px; height: 32px;"><label><input name="type" type="radio" onclick="document.getElementById('cknums').style.display='none';" id="RadioGroup1_0" value="0" checked="checked">单选</label>　
<label><input type="radio" <?php if($vo['cknums'] == 1): ?>checked<?php endif; ?> name="type" onclick="document.getElementById('cknums').style.display='block';" value="1" id="RadioGroup1_1">多选</label></p><p id="cknums"  style="display:none;width:200px; float: left"><input name="cknums" <?php if($vo['cknums'] != 1): ?>checked<?php endif; ?> class="px" size="4" type="text" value="<?php echo (($vo["cknums"])?($vo["cknums"]):1); ?>">多项最多选多少项
</p></td></tr><tr><th>截至时间：</th><td><input type="input" class="px" id="statdate" value="<?php if($vo['statdate'] != ''): echo (date("Y-m-d",$vo["statdate"])); endif; ?>" onClick="WdatePicker()" name="statdate">到
<input type="input" class="px" id="enddate" value="<?php if($vo['enddate'] != ''): echo (date("Y-m-d",$vo["enddate"])); endif; ?>" name="enddate" onClick="WdatePicker()"></td></tr><tr><th>投票结果：</th><td><label><input name="display" type="radio" <?php if($vo['display'] == 1): ?>checked<?php endif; ?>  value="1" id="RadioGroup2_1">投票前可见</label><label><input name="display" type="radio" <?php if($vo['display'] == 0): ?>checked <?php else: ?> checked<?php endif; ?> id="RadioGroup2_0" value="0">投票后可见</label><label><input name="display" type="radio" <?php if($vo['display'] == 2): ?>checked<?php endif; ?>id="RadioGroup2_2" value="2">投票结束可见</label></td></tr><tr><th>&nbsp;</th><td><button type="submit" name="button" class="btnGreen">保存</button><a href="<?php echo U('Vote/index');?>" class="btnGray vm">取消</a></td></tr></tbody></table></form></div></div></div></div></div><style>
.IndexFoot {
